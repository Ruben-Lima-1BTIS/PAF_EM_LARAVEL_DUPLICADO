<div x-data="{
...
}" x-show="open" x-transition.opacity x-cloak @keydown.escape.window="open = false"
    ...
    <div x-show="open" x-transition.scale class="relative bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        ...
        <form method="POST" :action="`/hour-approval/${hourId}/approve`" @submit="loading = true" class="flex gap-3">
            @csrf
